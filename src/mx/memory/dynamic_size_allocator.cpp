...
AllocationBlock::AllocationBlock(const std::uint32_t block_id, const std::uint8_t numa_node_id, const std::size_t size)
    : _id(block_id), _numa_node_id(numa_node_id), _size(size), _available_size(size)
{
    this->_allocated_block = GlobalHeap::allocate(numa_node_id, size);
    this->_free_elements.emplace_back(reinterpret_cast<std::uintptr_t>(this->_allocated_block), size);
    this->_lock.unlock();
}
                                   ...
                                   std::vector<AllocationBlock> &blocks, std::atomic<bool> &flag)
{
    ...
    if (can_allocate)
    {
        ...
        blocks.emplace_back(next_id, numa_node_id, size);
...
void Allocator::initialize_empty()
{
    ...
    for (auto i = 0U; i <= system::cpu::max_node_id(); ++i)
    {
        ...
        if (blocks.empty())
        {
            ...
            blocks.emplace_back(next_id, std::uint8_t(i), 4096U * 4096U);
        }
    }
}
