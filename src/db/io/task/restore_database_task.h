...
namespace db::io {
class RestoreDatabaseTask final : public mx::tasking::TaskInterface
{
public:
    RestoreDatabaseTask(const std::uint32_t client_id, topology::Database &database,
                        topology::Configuration &configuration, std::string &&file_name) noexcept
        : _client_id(client_id), _database(database), _configuration(configuration), _file_name(std::move(file_name))
    {
    }

    ~RestoreDatabaseTask() noexcept override = default;
