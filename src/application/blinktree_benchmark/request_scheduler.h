...
{
public:
    RequestContainer(const std::uint16_t worker_id, const std::uint64_t max_open_requests,
                     benchmark::Workload &workload) noexcept
        : _finished_requests(worker_id), _local_buffer(workload.next(config::batch_size())),
     ...
     */
    void inserted(const std::uint16_t worker_id, const std::uint64_t /*key*/, const std::int64_t /*value*/) noexcept
    {
     ...
     */
    void updated(const std::uint16_t worker_id, const std::uint64_t /*key*/, const std::int64_t /*value*/) noexcept
    {
        task_finished(worker_id);
    }
     ...
     */
    void removed(const std::uint16_t worker_id, const std::uint64_t /*key*/) noexcept { task_finished(worker_id); }

    /**
     * Callback after found a value.
     */
    void found(const std::uint16_t worker_id, const std::uint64_t /*key*/, const std::int64_t /*value*/) noexcept
    {
        task_finished(worker_id);
    }
     ...
     */
    void missing(const std::uint16_t worker_id, const std::uint64_t /*key*/) noexcept { task_finished(worker_id); }
...
private:
     ...
     */
    void task_finished(const std::uint16_t worker_id) { _finished_requests.add(worker_id); }
};
...
{
public:
    RequestSchedulerTask(const std::uint16_t worker_id, benchmark::Workload &workload,
        ...
        : _tree(tree), _listener(listener)
    {
        ...
        const auto container = mx::tasking::runtime::new_resource<RequestContainer>(
            sizeof(RequestContainer), mx::resource::annotation{worker_id}, worker_id,
            config::max_parallel_requests() / core_set.count_cores(), workload);
    ...
    mx::tasking::TaskResult execute(const std::uint16_t worker_id) override
    {
        ...
        auto &request_container = *mx::resource::ptr_cast<RequestContainer>(this->annotation().resource());
            ...
            for (auto i = next_requests.index(); i < next_requests.index() + next_requests.count(); ++i)
            {
                ...
                if (tuple == benchmark::NumericTuple::INSERT)
                {
                    task = mx::tasking::runtime::new_task<
                        ...
                        worker_id, tuple.key(), tuple.value(), _tree, request_container);
                    task->annotate(_tree->height() > 1U ? mx::tasking::annotation::access_intention::readonly
                                                        : mx::tasking::annotation::access_intention::write);
                }
                else if (tuple == benchmark::NumericTuple::LOOKUP)
                {
                    task = mx::tasking::runtime::new_task<
                        db::index::blinktree::LookupTask<std::uint64_t, std::int64_t, RequestContainer>>(
                        worker_id, tuple.key(), request_container);

                    task->annotate(mx::tasking::annotation::access_intention::readonly);
                }
                else if (tuple == benchmark::NumericTuple::UPDATE)
                {
                    task = mx::tasking::runtime::new_task<
                        db::index::blinktree::UpdateTask<std::uint64_t, std::int64_t, RequestContainer>>(
                        worker_id, tuple.key(), tuple.value(), request_container);
                    task->annotate(_tree->height() > 1U ? mx::tasking::annotation::access_intention::readonly
                                                        : mx::tasking::annotation::access_intention::write);
                }

                /// We do not annotate the root for prefetching,
                /// since the root will be likely in the cache.
                task->annotate(_tree->root());
        ...
        else if (next_requests.is_finished())
        {
            ...
            mx::tasking::runtime::delete_resource<RequestContainer>(this->annotation().resource());
