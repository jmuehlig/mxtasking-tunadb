...
public:
    ...
    [[nodiscard]] nlohmann::json node_addresses() const { return _root.template get<Node<K, V>>()->addresses(); }
...
protected:
    ...
    // Pointer to the root.
    alignas(mx::system::cache::line_size()) mx::resource::ptr _root;
                                                ...
                                                const bool is_root) const
    {
        ...
        return mx::tasking::runtime::new_resource<Node<K, V>>(
            config::node_size(),
            mx::resource::annotation{_isolation_level, _preferred_synchronization_protocol,
     ...
     */
    [[nodiscard]] static mx::resource::expected_access_frequency predict_access_frequency(const bool is_inner,
                                                                                          const bool is_root)
    {
        if (is_root)
        {
     ...
     */
    [[nodiscard]] static mx::resource::expected_read_write_ratio predict_read_write_ratio(const bool is_inner)
    {
        return is_inner ? mx::resource::expected_read_write_ratio::heavy_read
                        : mx::resource::expected_read_write_ratio::balanced;
    }
