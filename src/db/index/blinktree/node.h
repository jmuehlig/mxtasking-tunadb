...
public:
    ...
    {
    public:
        [[nodiscard]] static mx::tasking::PrefetchDescriptor for_traversal() noexcept
        {
            constexpr auto traversal_size = config::node_size() / 4U;
            return mx::tasking::PrefetchSize::make(mx::tasking::PrefetchDescriptor::PrefetchType::NonTemporal,
                                                   traversal_size);
        }

        [[nodiscard]] static mx::tasking::PrefetchDescriptor for_lookup() noexcept
        {
            constexpr auto lookup_size = config::node_size() / 4U;
            return mx::tasking::PrefetchSize::make(mx::tasking::PrefetchDescriptor::PrefetchType::NonTemporal,
                                                   lookup_size);
        }

        [[nodiscard]] static mx::tasking::PrefetchDescriptor for_update() noexcept
        {
            //            return mx::tasking::PrefetchCLMask{mx::tasking::PrefetchDescriptor{0b1}};
            return mx::tasking::PrefetchDescriptor{};
        }

        [[nodiscard]] static mx::tasking::PrefetchDescriptor for_insert() noexcept
        {
            return mx::tasking::PrefetchSize::make(mx::tasking::PrefetchDescriptor::PrefetchType::Write,
                                                   config::node_size() / 2U);
        }
    };
    ...
    [[nodiscard]] nlohmann::json addresses() const
    {
        auto node = nlohmann::json{};
        node["begin"] = std::uintptr_t(this);
        node["end"] = std::uintptr_t(this) + config::node_size();

        if (this->is_inner() || this->is_branch())
        {
            node["children"] = nlohmann::json{};
            for (auto i = 0U; i <= this->size(); ++i)
            {
                node["children"].emplace_back(this->_inner_node.separators[i].template get<Node<K, V>>()->addresses());
            }
        }
...
{
    auto high = static_cast<std::int16_t>(size()) - 1;
    auto mid = high >> 1U;
    ...
    do
    {
        ...
        mid = (low + high) >> 1U; // Will work for size() - 1 < max(std::int32_t)/2
    } while (low <= high);
