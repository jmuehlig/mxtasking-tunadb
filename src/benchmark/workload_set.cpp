    ...
    auto parse = [](auto &file_stream, std::vector<NumericTuple> &data_set) -> bool {
        ...
        while (file_stream >> op_name >> key)
        {
            if (op_name == "INSERT")
            {
                ...
                data_set.emplace_back(NumericTuple::INSERT, key, std::rand());
            }
            else if (op_name == "READ")
            {
                data_set.emplace_back(NumericTuple::LOOKUP, key);
            }
            else if (op_name == "UPDATE")
            {
                contains_update = true;
                data_set.emplace_back(NumericTuple::UPDATE, key, std::rand());
            }
        }
                               ...
                               const std::uint64_t mixed_deletes)
{
    ...
    {
        this->_data_sets[static_cast<std::uint8_t>(phase::FILL)].emplace_back(NumericTuple::INSERT, i + 1U,
                                                                              std::rand());
    }
    ...
    {
        this->_data_sets[static_cast<std::uint8_t>(phase::MIXED)].emplace_back(NumericTuple::INSERT, i + 1U,
                                                                               std::rand());
    }
