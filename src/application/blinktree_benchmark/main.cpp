 ...
 */
std::tuple<Benchmark *, mx::tasking::PrefetchDistance, bool> create_benchmark(int count_arguments, char **arguments);
...
int main(int count_arguments, char **arguments)
{
    ...
    {
        auto runtime = mx::tasking::runtime_guard{use_system_allocator, cores, prefetch_distance};
        benchmark->start();
    }
...
std::tuple<Benchmark *, mx::tasking::PrefetchDistance, bool> create_benchmark(int count_arguments, char **arguments)
{
    ...
    */
    ...
    argument_parser.add_argument("--latched")
        ...
        .implicit_value(true)
        .default_value(false);
    argument_parser.add_argument("--rtm")
        .help("Prefer restricted transactional memory for synchronization?")
        .implicit_value(true)
        .default_value(false);
    ...
    argument_parser.add_argument("--out-nodes")
        .help("Name of the file the tree node addresses will be written in json format.")
        .default_value(std::string(""));
    ...
    catch (std::runtime_error &e)
    {
        ...
        return std::make_tuple(nullptr, mx::tasking::PrefetchDistance{0U}, false);
    }
    ...
    else if (argument_parser.get<bool>("--rtm"))
    {
        preferred_synchronization_method = mx::synchronization::protocol::RestrictedTransactionalMemory;
    }
    ...
    auto *benchmark =
        new Benchmark(std::move(cores), argument_parser.get<std::uint16_t>("-i"), std::move(workload_files[0]),
                      ...
                      argument_parser.get<std::string>("--out-nodes"), argument_parser.get<bool>("--profiling"));
    ...
    return std::make_tuple(benchmark, prefetch_distance, argument_parser.get<bool>("--system-allocator"));
}