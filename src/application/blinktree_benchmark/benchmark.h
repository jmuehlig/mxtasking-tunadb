...
public:
    Benchmark(benchmark::Cores &&, std::uint16_t iterations, std::string &&fill_workload_file,
              ...
              std::string &&tree_file_name, std::string &&nodes_file_name, bool profile);
...
private:
    ...
    // Name of the file to write node addresses to.
    const std::string _nodes_file_name;

    // If true, use idle profiling.
    const bool _profile;

    // Number of open request tasks; used for tracking the benchmark.
    alignas(mx::system::cache::line_size()) std::atomic_uint16_t _open_requests = 0;

    // List of request schedulers.
    alignas(mx::system::cache::line_size()) std::vector<RequestSchedulerTask *> _request_scheduler;
